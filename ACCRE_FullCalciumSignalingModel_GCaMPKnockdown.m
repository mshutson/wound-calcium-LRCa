...
knockoutVal=0.(*percentage to keep for knockout (so 1 - this for percent knockdown)*);
knockoutList={"GCaMP"};
...
(*import cell pairs to save (will have to separate them out later)*)
pairs=Flatten@Import["cellPairs.m"];
...
styles=
Table[Style[{2,i},If[(MemberQ[ablatedPos,i]&&\[Tau]>framesBeforeWounding)||(positions[[i,1]]<range/2),Black,GrayLevel[Min[maxF,If[gcampRand,gcampRandVals[[i]],1.]*Rescale[timePoints[[\[Tau],i]],{minInt,maxInt*maxGCAMPVal},{minF,maxF}]]]]],{\[Tau],Length@timePoints},{i,numCells2D}];

animList=HighlightMesh[mesh,#,MeshCellStyle->{1->Directive[Opacity[0],Antialiasing->False]}]&/@styles;

frames=If[stamps,MapIndexed[Show[#1,Graphics[{White,Text[Style["t = "<>ToString[Round[\[CapitalDelta]t*(First[#2]-(framesBeforeWounding+1)),.1]]<>" s",White,18],{75,35}],White,Thickness@.025,Line[{{.75*range,.1*range},{.75*range+50,.1*range}}]}]]&,animList],animList];
...
Export["FullModel_param9_GCaMPKDN_PLCVaried_base0.tif",frames2D,"FrameRate"->10];
...
animate2Dcalc[soln_,stamps_?BooleanQ]:=
Block[{timePoints, minF, maxF, styles, animList, frames},
  timePoints=Join[newEQSoln[[;;,;;,3]],Table[soln[[i]]/.t ->\[Tau],{\[Tau],0.1,400.1,\[CapitalDelta]t},{i, numCells2D}]];
  
  styles=Table[Style[{2, i},If[MemberQ[ablatedPos,i]&&\[Tau]>framesBeforeWounding,Black,HotColor[Rescale[timePoints[[\[Tau],i]],{0,1},{0,1.5}]]]],{\[Tau],Length@timePoints},{i,numCells2D}];
...
frames2D=animate2Dcalc[test[[1]],False];

Export["FullModel_param9_GCaMPKDN_PLCVaried_freeCalcium_base0.tif",frames2D,"FrameRate"->10];

Export["pairSolutions.m",test[[2]]];
