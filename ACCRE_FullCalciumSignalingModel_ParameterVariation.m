...
extParams={r\[Mu]T,\[Tau]Heal,rPMCA,kPMCA,rSOC,\[Eta]NCX,nNCX,kNCX,ncxRat,rlkPM,\[Alpha],\[Alpha]0,kl,nl,Kc,\[Eta]IPR,\[Eta]SERCA,kSERCA1,kSERCA2,\[Eta]lkER,Bx,\[Eta]GJIP3,\[Eta]GJc,connectAblated};
...
(*Full PM Flux*)
jPM=\[Eta]NCX*(j\[Mu]T+jlkPM+jSOC-jPMCA-ncxRat*jNCX);
(*IPR flux terms*)
...
finalParamAssociation=<|r\[Mu]T->41.14497773461118`,\[Tau]Heal->5.25`,rPMCA->0.12`,kPMCA->0.1`,rSOC->0.15000000000000002`,\[Eta]NCX->10.`,nNCX->2.5`,kNCX->1.6`,rlkPM->0.00025`,\[Alpha]->0.9`,\[Alpha]0->0.1`,kl->0.05`,nl->5.`,Kc->0.4`,\[Eta]IPR->4.`,\[Eta]SERCA->5.`,kSERCA1->0.1`,kSERCA2->0.`,\[Eta]lkER->0.01`,Bx->5.`,\[Eta]GJIP3->4.`,\[Eta]GJc->2.`,connectAblated->0.025`,ncxRat->1.`|>;
...
"SOC"->(\[Eta]SOC->#*\[Eta]SOC),
"PMCA"->(\[Eta]PMCA->#*\[Eta]PMCA)|>&@knockoutVal;
...
gjRand=False; (*True = give each cell a random GJ parameter the tissue model. Value for a border determined by smallest value of the two cells that share that border*)
...
gcampRand=False; (*True = randomize the amount of GCaMP in each cell. The random scale will also scale the fluorescence values of the model output*)
...
varyParam={r\[Mu]T,\[Tau]Heal,rPMCA,kPMCA,rSOC,\[Eta]NCX,nNCX,kNCX,rlkPM,\[Alpha],\[Alpha]0,kl,ncxRat,Kc,\[Eta]IPR,\[Eta]SERCA,kSERCA1,\[Eta]lkER,Bx,\[Eta]GJIP3,\[Eta]GJc}[[ToExpression[Environment["SLURM_ARRAY_TASK_ID"]]]];

randExtParams={varyParam};
...
Export["FullModel_param9_"<>ToString[varyParam]<>"_Varied.tif",frames2D,"FrameRate"->10];
