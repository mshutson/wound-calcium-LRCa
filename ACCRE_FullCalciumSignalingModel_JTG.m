...
accre=ToExpression[Environment["SLURM_ARRAY_TASK_ID"]];
distSweep=Range[1./3.,1./2.,(1./2.-1./3.)/9][[accre]];

(*Get distances from center of mesh to each cell centroid*)
distances=Sqrt[(#[[1]]-range*distSweep)^2+(#[[2]]-range/2)^2]&/@PropertyValue[{mesh,2},MeshCellCentroid]*meshRatio;
...
tissueModelEquationsEQ=Flatten[Table[tissueCellEquationsEQ[[i]]/.If[positions[[i,1]]<2*range/3,knockoutRules,{}]/.If[positions[[i,1]]<range/2,overExpressRules,{}],{i,numCells2D}]]/.intParams;
tLong=300.; (*seconds*)
...
tissueModelEquations=Flatten[Table[tissueCellEquations[[i]]/.If[MemberQ[ablatedPos,i],{Subscript[c, i]'[t]==x_->Subscript[c, i]'[t]==0,Subscript[c, i][0]==x_->Subscript[c, i][0]==cExt},{}]/.If[positions[[i,1]]<2*range/3,knockoutRules,{}]/.If[positions[[i,1]]<range/2,overExpressRules,{}],{i,numCells2D}]]/.intParams;
tissueModelSol=
ParametricNDSolveValue[
...
Export["FullModel_param9_JumpTheGap_GCaMP_PLC_Varied_"<>ToString[accre]<>".tif",frames2D,"FrameRate"->10];
