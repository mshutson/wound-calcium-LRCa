...
groupedByDistance=SortBy[GatherBy[Delete[Range[numCells2D],ablatedPos/.x_?NumberQ->{x}],Round[distances[[#]]/\[CapitalDelta]x]&],distances[[First@#]]&];
distanceGroups=Mean/@(distances[[#]]&/@groupedByDistance);
...
Export["FullModel_param9_control_freeCalciumRun_GCaMP_base0.tif",frames2D,"FrameRate"->10];
...
frames2D=animate2Dcalc[test,False];

Export["FullModel_param9_control_freeCalciumRun_calcium_base0.tif",frames2D,"FrameRate"->10];

Export["EQs.m",newEQs];

groupedCalcium=Mean/@((test[[#]]&/@groupedByDistance)/.t->60.);
avgCavsDistance=Transpose@{Flatten@distanceGroups,Flatten@groupedCalcium};

Export["avgCavsDistance.m",avgCavsDistance];
